...
def _detect_text_label_cols(df: pd.DataFrame) -> Tuple[str, str]:
    """Infer text and label column names from a processed DataFrame.

    The function considers a small set of common column names and falls
    back to heuristics (e.g., selecting a low-cardinality column) when a
    clear match is not present. This keeps the script robust to minor
    schema differences between datasets.

    Returns:
        Tuple of (text_col, label_col).
    """
    candidates_text = [
        c for c in df.columns if c.lower() in ("text", "sentence", "excerpt", "bio")
    ]
    candidates_label = [
        c
        for c in df.columns
        if c.lower() in ("label", "label_id", "target", "profession", "class")
    ] + ["gold_label"]
    ...
    if label_col is None:
        raise ValueError(
            "Could not infer label column from processed CSV. Please check headers."
        )
...
def load_processed_splits(base_path: str = "data/processed"):
    ...
    val = (
        val[[tx, ly]].rename(columns={tx: "text", ly: "label"})
        if tx in val.columns and ly in val.columns
        else val
    )
...
def sanitize_text(x) -> str:
    """Normalize text values into safe strings for tokenization.

    Converts nan/None to empty string and strips whitespace. This prevents
    downstream tokenizers from failing on unexpected types.
    """
...
def build_gender_lexicon() -> Dict[str, str]:
    """Build a simple bidirectional mapping of gendered word pairs.

    The lexicon is used for Counterfactual Data Augmentation (CDA) where
    gendered terms are swapped to produce mirrored examples. The mapping
    includes common pronouns and role nouns. It is intentionally simple
    (not context-aware) and therefore complemented by light heuristics
    (see ``swap_gender_words``) to avoid obviously incorrect substitutions.

    Returns:
        Dict mapping lowercased token -> swapped token.
    """
...
def simple_token_words(text: str) -> List[str]:
    ...
    This function preserves whitespace tokens so that reconstructed text
    can maintain original spacing and punctuation when we perform token
    level replacements (used by ``swap_gender_words``).
    """
...
def swap_gender_words(text: str, lex: Dict[str, str]) -> str:
    # Tokenize while preserving spaces/punctuation so we can rebuild the
    # string exactly. For word tokens we consult the lexicon; for 'her'
    # we apply a small heuristic to choose 'his' vs 'him' depending on the
    # syntactic context (very crude but reduces some incorrect swaps).
    ...
    while i < len(tokens):
        ...
        if re.match(r"^\w+$", tok):
            ...
            if low == "her":
                ...
                repl = (
                    "his"
                    if j < len(tokens) and re.match(r"^\w+$", tokens[j])
                    else "him"
                )
                if tok.istitle():
                    repl = repl.title()
...
def cda_augment(
    train_texts: List[str], train_labels: List[int], only_if_present: bool = True
) -> Tuple[List[str], List[int]]:
    """Apply Counterfactual Data Augmentation by swapping gendered terms.

    For each training example we produce either the original example only
    (when ``only_if_present`` is True and no gendered token was found) or
    both original + swapped example. The label is kept the same for the
    augmented example (typical CDA strategy for fairness augmentation).

    Args:
        train_texts: List of training strings.
        train_labels: Corresponding integer labels.
        only_if_present: When True, only augment examples that contain a
            gendered token; otherwise augment all examples by creating a
            swapped copy even if no token changed.
            ...
            rows.append(
                {
                    ...
                    **{
                        k: (
                            ...
                            else (metrics if k == "precision" else None)
                        )
...
def build_tokenized_datasets(tokenizer, X_train, y_train, X_val, y_val, X_test, y_test):
    """Tokenize raw text lists into HuggingFace Dataset objects ready for Trainer.

    The function ensures text values are sanitized and returns tokenized
    datasets with columns set for torch formats (input_ids, attention_mask, label).
    """
...
def main():
    ...
    X_train_aug, y_train_aug = cda_augment(
        X_train_raw, list(y_train), only_if_present=True
    )
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        MODEL_ID, num_labels=len(le.classes_)
    )

    tok_train, tok_val, tok_test = build_tokenized_datasets(
        tokenizer,
        X_train_aug,
        y_train_aug,
        X_val_raw,
