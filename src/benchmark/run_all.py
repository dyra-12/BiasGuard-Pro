...
def run_model_reports():
    ...
    mod = (
        importlib.import_module("src.evaluate".replace("/", ".").replace(".py", ""))
        if False
        else None
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Unified experiment runner for BiasGuard Pro"
    )
    p.add_argument(
        "--stage",
        choices=["all", "model_reports", "cross_dataset", "stats_tests"],
        default="all",
        help="Which stage to run (default: all)",
    )
    # Legacy / compatibility flags (ignored but accepted)
    p.add_argument(
        "--models", default="all", help="(compat) Models spec; ignored (auto-discovery)"
    )
    p.add_argument(
        "--dataset",
        default="biasbios_test",
        help="(compat) Dataset name; internal scripts use fixed paths",
    )
    p.add_argument(
        "--output",
        default=str(RESULTS_DIR),
        help="(compat) Output directory; scripts write to results/",
    )
    p.add_argument(
        "--model-path", default="models", help="Path to main BiasGuard model directory"
    )
    return p.parse_args()
