...
def load_test_data(path: str, text_col: str, label_col: str) -> pd.DataFrame:
    ...
    if text_col not in df.columns or label_col not in df.columns:
        raise ValueError(f"Missing required columns '{text_col}' or '{label_col}' in {path}")
    return df
...
def robust_load_roberta(model_dir: str) -> Optional[Tuple[Any, Any]]:
    ...
    vocab_size = len(tokenizer) if hasattr(tokenizer, "__len__") else getattr(config, "vocab_size", None)
...
def try_load_sklearn_model(model_path: str, vectorizer_path: Optional[str] = None):
    try:
        ...
        vect = joblib.load(vectorizer_path) if vectorizer_path and Path(vectorizer_path).exists() else None
        return model, vect
    except Exception as e:
        print(f"[WARN] Sklearn model load failed ({model_path}): {e}")
        return None, None

def predict_sklearn(model, vectorizer, texts: List[str]) -> Tuple[np.ndarray, Optional[np.ndarray]]:
...
def read_predictions_csv(pred_csv: str) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    ...
    pred_col = next((c for c in ["pred", "prediction", "y_pred", "label_pred", "predicted_label"]
    ...
    proba_col = next((c for c in ["proba", "prob", "prob_pos", "probability", "score"]
                      if c in df.columns), None)
...
def classification_metrics(y_true: np.ndarray, y_pred: np.ndarray, y_proba: Optional[np.ndarray]) -> Dict[str, Any]:
...
def paired_t_test_trueclass_proba(y_true: np.ndarray, proba_a: np.ndarray, proba_b: np.ndarray) -> Dict[str, Any]:
...
def compute_fairness(y_true: np.ndarray, y_pred: np.ndarray, sensitive: pd.Series) -> Dict[str, Any]:
    ...
    for g in groups:
        ...
        if cm.size == 4:
            ...
            fpr = 0.0
        prec = precision_score(yt, yp, pos_label=1, zero_division=0) if len(yp) else np.nan
...
def build_baselines(tfidf_model: str, tfidf_vect: str, tfidf_csv: str,
                    glove_model: str, glove_csv: str,
                    roberta_dir: str, cda_dir: str) -> Dict[str, Dict[str, Any]]:
...
def run_evaluation(test_csv: str, text_col: str, label_col: str,
                   biasguard_dir: str, baselines: Dict[str, Dict[str, Any]],
                   sensitive_cols: List[str], out_dir: str) -> None:
    ...
    if biasguard_loaded is None:
        raise RuntimeError(f"BiasGuard Pro failed to load from {biasguard_dir}. Check directory and files.")
    ...
    def evaluate_baseline_local(name: str, cfg: Dict[str, Any]) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        if cfg["type"] == "hf":
    ...
    for base_name, base_cfg in baselines.items():
        ...
        if ttest_res:
            row.update({
                ...
                "paired_t_mean_diff_trueclass_proba": ttest_res["mean_diff_trueclass_proba"],
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run paired statistical tests between BiasGuard and baselines.")
    ...
    p.add_argument("--label-col", default=LABEL_COL, help="Label column name in test CSV")
    p.add_argument("--biasguard-dir", default=BIASGUARD_DIR, help="BiasGuard model directory")
    ...
    p.add_argument("--tfidf-model", default=TFIDF_MODEL, help="TF-IDF model joblib path")
    p.add_argument("--tfidf-vect", default=TFIDF_VECTORIZER, help="TF-IDF vectorizer joblib path")
    p.add_argument("--tfidf-csv", default=TFIDF_PREDICTIONS_CSV, help="TF-IDF predictions CSV (fallback)")
    p.add_argument("--glove-model", default=GLOVE_SVM_MODEL, help="GloVe SVM model joblib path")
    p.add_argument("--glove-csv", default=GLOVE_SVM_PREDICTIONS_CSV, help="GloVe predictions CSV (preferred)")
    p.add_argument("--out-dir", default=OUT_DIR, help="Output directory for results")
    p.add_argument("--sensitive-cols", nargs="*", default=SENSITIVE_COLS, help="Sensitive columns to compute fairness on")
    return p.parse_args()
